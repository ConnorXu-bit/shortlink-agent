...
**同一轮的多个工具并发执行**：一轮里模型可能同时要求创建 3 个短链，这些调用彼此
独立、又都是 I/O 密集型（Redis 往返），所以 `_run_tool_calls` 用线程池并发执行，
再用 `pool.map` 保证结果顺序与 `tool_calls` 一一对应。实测 4 个各耗时 0.25 秒的工具，
总耗时从 1.02 秒降到 0.26 秒。

**这个设计的代价**：每次调用都要重发完整上下文，所以总开销随轮数呈**平方级增长**
（实测 1 轮 410 字符、3 轮 2132 字符、5 轮 5166 字符），延迟也从两次往返叠加到六次。
轮数上限因此是**保险丝而不是正确性保证**：它只保证不会死循环，不保证任务一定完成。
